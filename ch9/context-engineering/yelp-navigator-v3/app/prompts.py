# v3/prompts.py
"""Re-export shared prompts and add memory-aware variants."""
...
def clarification_system_prompt(
    ...
    known_businesses: List[Dict[str, str]] = None
) -> str:
    """
    Generates a state-aware clarification prompt with memory awareness.
    It knows about the *previous* search and cached businesses to better handle follow-ups.
    """
    ...
    memory_context = ""
    if known_businesses:
        business_list = ", ".join([f"'{b['name']}' (ID: {b['id']})" for b in known_businesses[:5]])
        memory_context = f"""
--- KNOWN BUSINESSES IN MEMORY ---
I have information cached about these businesses from recent searches:
{business_list}

If the user is asking about any of these businesses specifically, I can retrieve
detailed information from memory without needing to search again.
"""
...
2.  **If the user is asking about a specific business from the known businesses:**
    - This is a follow-up question. Keep the current QUERY and LOCATION.
    - Set DETAIL_LEVEL based on what they're asking for.

3.  **If the user is asking for a *completely new search*:**
    - Extract the new QUERY and LOCATION.
    - Set DETAIL_LEVEL based on their new query (default to "general").
    
4.  **If the user is just chatting:**
...
NOTE: The system has memory capabilities. If data was previously cached, it may already be available.
"""
